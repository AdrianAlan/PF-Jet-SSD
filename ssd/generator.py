...
class CalorimeterJetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        while redo:
            ...
            calorimeter = calorimeter.reshape(self.channels,
                                              self.height,
                                              self.width)
            calorimeter_max = calorimeter.reshape(
                self.channels, -1).max(axis=1).reshape(
                self.channels, 1, 1) + 1e-12
            calorimeter = calorimeter / calorimeter_max
            # calorimeter = np.rollaxis(calorimeter, 0, 3) # channels last
            calo = np.zeros((2, 300, 300))
            calo[0] = cv2.resize(calorimeter[0], dsize=(300, 300),
                                 interpolation=cv2.INTER_CUBIC)
            calo[1] = cv2.resize(calorimeter[1], dsize=(300, 300),
                                 interpolation=cv2.INTER_CUBIC)
